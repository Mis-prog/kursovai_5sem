...
/** \class Ref
  ...
  * \code
  * // in-out argument:
  * void nodeFillCoef(Ref<VectorXf> x);
  *
  ...
  * MatrixXf A;
  * VectorXf a;
  * nodeFillCoef(a.head());             // OK
  * nodeFillCoef(A.col());              // OK
  * nodeFillCoef(A.row());              // Compilation error because here innerstride!=1
  ...
  * foo3(A.row());              // OK
  * \endcode
  * The downside here is that the function foo3 might be significantly slower than nodeFillCoef because it won't be able to exploit vectorization, and will involve more
